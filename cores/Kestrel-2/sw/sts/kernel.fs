\ Milestone-4.fs is a copy of Milestone-2 with support for loading
\ slide data from SD card.  Well, eventually.  For now, it was a
\ challenge just getting the bootstrap to work right.  :)

$0400 origin
defer, bootstrap

:, hlt		$C000 #, @, 1 #, +, $C000 #, !, again, ;,
...
:, sleep	ctr @, -1 #, +, ctr !,  ctr @, if, again, then, ;,
:, delay	3 #, ctr !, sleep ;,
...
:, out		spi c@, $80 #, and, mosi  spi c@, spi c@, +, spi c!, ;,
:, inp		miso if, spi c@, 1 #, xor, spi c!, then, ;,
:, edge		gpop @, kCLK1 xor, gpop !, ;,
:, clk		delay edge ;,
:, b		out inp clk clk ;,
:, io		b b b b b b b b ;,
:, iow		spiw 1 #, +, c@, spi c!, io   spiw c@, spi c!, io ;,
:, send		cmdbyte c@, spi c!, io paramh @, spiw !, iow paraml @, spiw !, iow ( crc ) io ;,
:, (wait)	toctr @, if,
...
:, param0	0 #, paramh !,  0 #, paraml !, ;,
char, _r1
:, in		$FF #, spi c!, io spi c@, ;,
:, r1		in _r1 c!, ;,
:, -ready?	_r1 c@, ;,
:, start	in $FF #, xor, if, ;, then, again, ;,
:, bytes	bytectr @, if, in to @, c!,  1 #, to @, +, to !,  -1 #, bytectr @, +, bytectr !, again, then, ;,
:, (data)	512 #, bytectr !, bytes ;,
:, 512bytes	start (data) in in ;,
:, data		-ready? if, ;, then, 512bytes ;,
:, cmd17	cs0 $51 #, cmdbyte c!, param0 from @, paraml !, issue -timeout? if, r1 data then, cs1 in ;,
:, (led0)	gpop @, kLED0 and, ;,
:, led0		(led0) gpop !, ;,
:, led1		(led0) kLED1 xor, gpop !, ;,
:, sector	led1 cmd17  from @, 512 #, +, from !, led0 ;,
:, block	sector sector ;,
...
\ int, aa
\ int, bb
\ :, mv		count @, if, aa @, @, bb @, !,  aa @, 2 #, +, aa !,  bb @, 2 #, +, bb !,  -2 #, count @, +, count !, again, then, ;,
\ :, vis		buf aa !,  $E000 #, bb !,  1024 #, count !,  mv ;,

:, count	SDA_BASE from !,  buf to !,  block buf @, nslides !, ;,
:, dir		SDA_BASE from !, buf to !, block ;,
:, start	dir  cslide @, cslide @, +, 2 #, +, buf +, @, SDA_BASE +, from !, buf to !, block  buf ip !, ;,
